...
bot.onText(/\/start/, async (msg) => {
  ...
  bot.sendMessage(
    chatId,
    "🔥 *Студия декора FBRiQ - ваш надежный партнер в мире прочных решений и впечатляющих конструкций. Добро пожаловать.* 🔥",
    options
  );
});
...
if (query.data.startsWith('expense_sheet_')) {
 ...
 if (query.data === "download_presentation") {
  ...
  try {
    ...
    const buffer = response.data;
    return bot.sendDocument(chatId, buffer, {}, { filename: "Студия декора FBRiQ" });
  } catch (error) {
  ...
  const aboutText = `*Студия декора FBRiQ* — это команда профессионалов, создающих впечатляющие конструкции и декорации для бизнеса, рекламы и ивентов. Мы воплощаем смелые идеи, превращая пространство в эффектные и запоминающиеся локации.
